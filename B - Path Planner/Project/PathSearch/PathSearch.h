...
namespace fullsail_ai { namespace algorithms {
	...
	private:
		...
		struct PlannerNode
		{
			...
			//costs
			float heuristicCost;//Greedy Best-First Search// = estimate(start, goal);
			float givenCost;//Uniform Cost Search// = current.givenCost + edge.cost;
			float finalCost;//A* Search Planning// = givenCost + heuristicCost * hWeight;

		public:
			//set and get funtions
			float heuristicCostEQU(float start, float goal)
			{
				return start - goal;
			}

			float givenCostEQU(float cgivenCost, float ecost)
			{
				return cgivenCost + ecost;
			}

			float finalCostEQU(float givenCost, float heuristicCost, float hWeight)
			{
				return givenCost + heuristicCost * hWeight;
			}

			void setHeuristicCost(float heuricCost)
			{
				heuristicCost = heuricCost;
			}
		...
		//Below is a simple example of using a std::map to add a key - value pair – in this
		//case, a PlannerNode at the specified vertex :
		//std::unordered_map<SearchNode*, PlannerNode*> myPlannerNode;
		...
		//pair<double, double> goalHeuristic;
		float hWeight;
		//int rows, columns;
		std::vector<Tile const*> solutionList;
		PriorityQueue<PlannerNode*, compare> open;
		bool isFinish;
		SearchNode* goalTile;
		SearchNode* firstTile; //start and goal
		//OwnTimer myTimer;
